...
def build_table(prefix):
    ...
    for i in range(rows):
        ...
        # ✅ dropdown (helper only)
        selected_val = col2.selectbox(
            "",
            [""] + [f"{x:.2f}" for x in options],
            key=f"{prefix}_dd_{i}"
        )

        # ✅ apply dropdown → sets amount
        if selected_val != "":
            st.session_state[amount_key] = float(selected_val)

        # ✅ SINGLE FIELD (REAL VALUE)
...
with d2:
    ...
    if same:
        ...
        st.info("Actual uses Declared values")
    else:
