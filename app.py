    ...
    for i in range(rows):
        ...
        if selected == "OTHER":
            ...
            for j in range(3):
                ...
                benefit = col1b.text_input("", key=f"{prefix}_custom_{i}_{j}", placeholder=f"Enter new benefit {j+1}")

                amount_val = col2b.number_input("Amount ($)", value=None, step=0.01,
                                                key=f"{prefix}_manual_other_{i}_{j}", format="%.2f")
...
c2.markdown(f"**Budget:** ${budget:,.2f}")
